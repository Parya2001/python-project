...
def get_filename():
    #Return filename with extension (.txt or .md)
    print(
...
Do your want to write the following to the output file as well?
    ...
    - WordsbywordMeaning: Exact meaning of the transliterated words
    - purport: Extensive explanation of the verse.
...
def getC_title(html, chap_num):
    try: return html.find('3', href=f"/chapter/{chap_num}").text
...
def get_originalVerse(html):
    ...
    try: return html.find('div', id="wrapper-devanagari").text.strip()
...
def get_translation(html):
    ...
    try:
        return html.find('div', id="wrapper-translation").text.strip()
    except Exception:
        return "TRANSLITERATION VERSION NOT FOUND."


def get_wordbywordMeanings_en(html):
    """Extract Meaning of the transliteration version of the words in English"""
    try:
        return html.find('div', id="wrapper-synonyms").text.replace(
            '; ', '\n> - ').replace('—', ' — ').strip()
...
def get_translation_en(html):
    ...
    def get_bg(html):
        ...
        try: return html.find('div', id="wrapper-translation").span.text
        except Exception: return ""
    try:
        return html.find(
            'div', id="wrapper-translation").text.strip().replace(get_bg(html), '').lstrip()
    except Exception:
        return "ENGLISH VERSION NOT FOUND."


def get_purport_en(html):
    """Retrieving commentary on the verse"""
    try:
        return html.find('div', id="wrapper-puport").text.strip()
    except Exception: return "PURPORT NOT FOUND."
